...
async function createGist(content) {
    try {
        const resp = await fetch("https://my-worker-simple.daniel-a-bernard.workers.dev/", {
            method: "POST",
        ...
        if (!resp.ok) {
            return { error: `Worker returned status ${resp.status}` };
        }

        const data = await resp.json();
        return data; // { url: "https://gist.github.com/..." }
    } catch (err) {
...
async function listCommand(filters = "") {
    ...
    // On prend seulement les noms des jumps
    ...
    // Appel sécurisé au Worker GitHub
    const res = await createGist(output);
    if (res.url) return `Liste créée: ${res.url}`;
    else return `Erreur: ${res.error || "Gist non créé."}`;
}
